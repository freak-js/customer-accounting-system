...
def create_service_for_client(request) -> dict:
    if Service.validate_pk_in_form(request):
        p = request.POST
        ...
        service = Service(client=client, cash_machine=cash_machine, ecp=ecp, ecp_add_date=ecp_add_date,
                          ecp_expiration_date=ecp_expiration_date, ofd=ofd, ofd_add_date=ofd_add_date,
                          ofd_expiration_date=ofd_expiration_date, fn=fn, fn_add_date=fn_add_date,
                          fn_expiration_date=fn_expiration_date, to=to, to_add_date=to_add_date,
                          to_expiration_date=to_expiration_date)
        service.save()
        print('УДАЧНО СОХРАНИЛСЯ')
        return {'error': False}
    else:
        return {'error': True}
